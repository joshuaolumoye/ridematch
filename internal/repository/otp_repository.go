...
// challenges, keyed by identifier (a normalized phone number or email
// address — see models.OTPRequest.Identifier).
type OTPRepository interface {
	...
	// not consumed) OTP issued for an identifier and purpose.
	FindLatestActive(ctx context.Context, identifier string, purpose models.OTPPurpose) (*models.OTPRequest, error)
	// FindLatestAny returns the most recent OTP regardless of validity,
	// used to enforce the resend cooldown.
	FindLatestAny(ctx context.Context, identifier string, purpose models.OTPPurpose) (*models.OTPRequest, error)
...
func (r *otpRepository) FindLatestActive(ctx context.Context, identifier string, purpose models.OTPPurpose) (*models.OTPRequest, error) {
	var otp models.OTPRequest
	err := r.db.WithContext(ctx).
		Where("identifier = ? AND purpose = ? AND consumed_at IS NULL AND expires_at > ?", identifier, purpose, time.Now()).
...
func (r *otpRepository) FindLatestAny(ctx context.Context, identifier string, purpose models.OTPPurpose) (*models.OTPRequest, error) {
	var otp models.OTPRequest
	err := r.db.WithContext(ctx).
		Where("identifier = ? AND purpose = ?", identifier, purpose).
		Order("created_at DESC").
		First(&otp).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, ErrNotFound
